...
const RecipeList: FC = () => {
  ...
  const RecipeListContent = recipeData?.pages.map(list => list.COOKRCP01.row).flat()
  ...
  return (
    <>
      <div className=" relative ml-[13px]  w-full px-[450px] py-[50px] text-black">
        <Grid container spacing={{ xs: 1, md: 2, lg: 3 }} style={{ paddingBottom: 20 }}>
          {RecipeListContent &&
            RecipeListContent.map((recipe, index) => (
              <Grid xs={6} md={4} lg={3} style={{ paddingBottom: 40 }} key={index + 1}>
                <OneRecipe
