...
export function useGetRecipeInfinity() {
  ...
  const {
    ...
    queryFn: ({ pageParam = { startIdx: 1, endIdx: 12 } }) => getRecipe({ ...pageParam, RCP_PAT2: "반찬" }),
 ...
 */
export function useGetDetailRecipe({ ...recipeKey }: { startIdx: number; endIdx: number; RCP_NM: string }) {
