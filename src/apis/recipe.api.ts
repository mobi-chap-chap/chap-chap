...
export const getDetailRecipe = async ({ ...recipeData }: { startIdx: number; endIdx: number; RCP_NM: string }) => {
  const res = await axiosRecipeInstance.get<Info>(DETAIL_END_POINT.RECIPE({ ...recipeData }))
  return res.data
}
